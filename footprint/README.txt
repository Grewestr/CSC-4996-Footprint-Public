...
Welcome to the **Footprint Web Application** repository! This document provides comprehensive instructions for setting up, running, and testing the Footprint application in your local environment. Follow these steps to ensure proper functionality.
...
2. Clone the **Integration** branch of the GitHub repository:
   ```bash
   git clone --branch main git clone --branch Integration https://github.com/TKosinski24/CSC-4996-Footprint.git
   cd <repository-directory>
   ```

### Step 2: Create a Virtual Environment
1. Create a virtual environment in the project directory (but outside of the git repository) so one directory above CSC-4996-Footprint:
   ```bash
   python -m venv venv
   ```
...
1. Install the dependencies listed in `requirements.txt` (located in the footprint directory):
   ```bash
...
### Recommended Tests
1. Verify the application UI renders correctly across supported browsers.
2. Upload videos at different processing speeds (slow, medium, fast).
3. Check real-time updates in the upload queue and dashboard.
...
### Logs and Debugging
- Check Docker and Django logs for detailed error messages. enable logging on line 166 of setting.py:
  ```bash
  docker-compose logs
  python manage.py runserver
  ```
